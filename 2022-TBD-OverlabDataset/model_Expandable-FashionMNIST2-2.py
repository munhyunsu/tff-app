...
class FLModel(model.Model):
    def __init__(self, **kwargs):
        self.name = 'Expandable-FashionMNIST2-2'
        self.label = ['Ankle boot', 'Bag', 'Coat', 'Dress', 'Pullover', 'Sandal']
        ...
        idx = -1
        with os.scandir('./models/scenario1') as it:
            for entry in it:
                if entry.name.startswith('v') and entry.is_dir():
                    tidx = int(entry.path.split('.')[-1])
                    if idx < tidx:
                        idx = tidx
        fork_model = tf.keras.models.load_model(f'./models/scenario1/v0.0.{idx}')
