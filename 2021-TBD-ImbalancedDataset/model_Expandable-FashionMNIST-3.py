...
class FLModel(model.Model):
    def __init__(self, **kwargs):
        self.name = 'Expandable-FashionMNIST-3'
        # ['T-shirt', 'Trouser', 'Pullover', 'Dress', 'Coat', 'Sandal', 'Shirt', 'Sneaker', 'Bag', 'Ankle boot']
        self.label = ['T-shirt', 'Trouser', 'Shirt', 'Dress', 'Ankle boot', 'Sandal', 'Sneaker']
        ...
        idx = -1
        with os.scandir('./models/scenario2') as it:
            for entry in it:
                if entry.name.startswith('v') and entry.is_dir():
                    tidx = int(entry.path.split('.')[-1])
                    if idx < tidx:
                        idx = tidx
        fork_model = tf.keras.models.load_model(f'./models/scenario2/v0.0.{idx}')
